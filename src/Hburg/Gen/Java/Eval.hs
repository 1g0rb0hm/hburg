...
import Hburg.Ast.Attr (attrGetIn, attrGetOut, attrId, attrTy)
import Hburg.Ast.Term (TermClass(..))
import Hburg.Ast.Node (Node, Position(..), mapPreOrder3, getSemAct, hasLink, getLink)
import Hburg.Ast.Prod (getRuleLabel, getNode)
import Hburg.Ast.Def (Definition, getProds, getCode)

import Hburg.Gen.Java.Modifier (Modifier(..))

{- qualified imports  -}
import qualified Hburg.Ast.Code as C (Code, isEmpty)
import qualified Hburg.Ast.Ir as Ir (Ir(..))

import qualified Hburg.Gen.Ident as I (Ident, rId, nId, nTy, rTy)
import qualified Hburg.Gen.Label as Lab (termToEval, termToEnum, childCall)
import qualified Hburg.Gen.Java.Class as Class (Class(..), new)
import qualified Hburg.Gen.Java.Method as M (Method, new)
import qualified Hburg.Gen.Java.Param as P (Param, new)
...
{- | Generates all evaluation methods which emit code supplied by the user
     in semantic actions. -}
evalMethods :: I.Ident -> [Definition] -> [M.Method]
evalMethods ids defs =
  map (\d ->
        let ps = (P.new (I.nTy ids) (I.nId ids)) : (params d) in
        M.new Private True (returnType d) (Lab.termToEval d) ps (body d))
      (defs)
  where
    ...
    body d = t (I.rTy ids) <+> t (I.rId ids) <+> equals -- 1.
        <+> t (I.nId ids) <> t ".rule" <> parens (t $ Lab.termToEnum d) <> semi
      $+$ returnVar d                                   -- 2.
        ...
        cases def =
          map
            (\p ->
                let childCalls =
                      mapPreOrder3
                        (\pos  n -> [t "." <> t (Lab.childCall pos) <> parens (empty)])
          ...
          where
            ...
            methodCall term path | (isNonTerminal term) =
              ...
              t (Lab.termToEval term)              -- function name
