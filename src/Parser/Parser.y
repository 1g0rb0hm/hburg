...
import Parser.Messages (parseErrDupBind, parseErrTok, parseErrRedef)
...
import qualified Ast.Prod as P (Production, new)
import qualified Ast.Cost as Cost (Cost, static, dynamic)
      ...
      rules
        ...
        {%
          ...
          let entries = (map (\d -> Debug.new Debug.Debug (show d)) defs) -- debug entries
                ...
                ++ [Debug.new Debug.Debug ("Operator-" ++ show opctx)] in
...
Prod :: { (P.Production, OperatorMap) }
    ...
    | Sem T Sem '[' Sem Nt Sem ']' Sem ':' Cost
        {%
          ...
          let p = P.new (N.setLink (N.addLinkCode n $5 $7) link) $11 in
