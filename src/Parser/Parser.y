...
-- semantic checking, e.g. duplicate bindings, etc.
--
-----------------------------------------------------------------------------
...
import qualified Ast.Bind as B (new, empty, getBinding)
	...
	| Sem T Sem '[' Sem Nt Sem ']' Sem ':' cost
		{% 
			...
			if (equalBindings $2 $6)
				then errP 
							(parseErrDupBind "Binding" 	(envElem (B.getBinding (getBinding $6))) 
													 	(envElem (B.getBinding (getBinding $2))))
							(p)
				else returnP p
		}
