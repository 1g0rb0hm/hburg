...
--
--
-- This module generates tiling target source code and the appropriate node interface.
-- 
--
-----------------------------------------------------------------------------
...
import Ast.Prod (Prod, getName, getCost, getNode,
	getRuleLabel, getResultLabel, getArity)
...
-- | genTiling. Top level function for generating tiling target source code 
--		and the appropriate node interface.
...
genTiling ops defs nkind parentClass
	...
	let (vars, sets) = genEnumSetVars tile in			-- Calculate EnumSet variables
	let nodeIf = genNodeInterface 						-- Node interface
						...
						nkind							-- return type of node kind
		in
	let j2 = jSetVariables j1 vars in
	let mLab = genLabelMethod tile in					-- Generic label() mehtod
	let mTile = genTileMethod tile sets in				-- tile() method as in in [Cooper p.566]
	let mLabS = genLabelSetMethods defs tile in			-- label_X() methods for nodes with arity X
	let nestedClass = jSetMethods j2					-- Finally set all methods for this tiling class
			( [mLab, mTile] ++ mLabS ++ 
				if (T.getClosures tile /= [])		-- only generate closure() method if there are closures
					then [genClosureMethod tile]
					else [])
		in
	(jSetNestedClasses parentClass [nestedClass],		-- the Tiling class is nested into another class
	nodeIf)												-- the generated node interface class


-- | genSetName. Generate a name for node sets based on node arity.
genSetName :: Int -> String
genSetName arity = "arity" ++ (show arity) ++ "Set"

-- | genLabelMethodName. Generate a name for the label method depending
--	on the arity of the nodes.
genLabelMethodName :: Int -> String
genLabelMethodName arity = "label_" ++ (show arity)


-- | genEnumSetVars. Produce EnumSet variables e.g.:
--		 private static EnumSet unarySet = EnumSet.of(FUN,FUNAP,SIDEFFECT)
...
genEnumSetVars tile
	...
	where
		...
		-- | transformOpSet. Make String representation from an Operator set
...
-- | genLabelMethod. Generates the label method
...
-- | genClosureMethod. Generate closure method
...
-- | genTileMethod. Generates the tile() method as in [Cooper p.566]
...
genTileMethod tiling sets
	...
	where
		...
		-- ifCascade. Generate the 'if ... else if ... else' cascade in the tiling() method
...
-- | genLabelSetMethods. Generate methods which do the actual labelling
--		of AST nodes.
...
genLabelSetMethods defs tiling
	...
	map -- Iterate over all arities and generate methods
		...
		(M.keys funmap)
	where
		-- | genLabelSetMethod. Generate method for
		...
		genLabelSetMethod arity prods
			...
			where
				...
				-- | genCases. Generate case stmts. for label methods
				genCases :: String
				genCases
					= M.fold
						(\prods old ->
				...
				-- | genProdMap. Generate map holding productions keyed by arity
				...
				-- | costAndLabel. Create "cost = ...;" and "label(...)" calls.
				costAndLabel :: Prod -> String -> String
				costAndLabel p indent
					= let childCalls = N.mapChildren
					...
					concatMap 
						(\(call, n) ->  
								...
								(case getNodeReturnType defs n of
									...
									Nothing -> error "\nERROR: costAndLabel() Node has no return type!\n") ++
								") + "  )
						(childCalls) ++
					show (getCost p) ++ ";\n" ++
					-- Call label
				...
				-- | iff. Produce code which is used within an if to evaluate if
				--		a certain node should be labeled.
				--		e.g.: if (n.child1().kind() = ADD && n.child1().left().is(NT_REG) ...)
				iff :: Prod -> String
				iff p
					= let childCalls = N.mapPreOrder2
