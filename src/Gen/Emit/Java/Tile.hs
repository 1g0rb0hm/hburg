...
import qualified Ast.Node as N (Node, mapPreOrder2)
...
tileMethod ids linkset sets =
  M.new Public True "void" "tile" [Param.new (I.nTy ids) (I.nId ids)] body
  where
      ...
      <+> lbrace
        $+$ nest 2 (
              vcat [ t "tile" <> parens ( t (I.nId ids) <>
                     t ('.':(childCallLab pos)) <> parens empty <> semi)
                   | pos <- [1..x]]
...
labMethods ids ir prodmap =
  ...
  where
    ...
    labSetMethod arity prods =
      M.new Private True "void" (render $ labMethodName arity) [Param.new (I.nTy ids) (I.nId ids)] body
      where
        {- | Method body -}
        body :: Doc
        ...
        simpleCase p = t "case" <+> t (termToEnumLab p) <> colon
          <+> lbrace
        ...
        complexCase prods = t "case" <+> t (termToEnumLab . head $ prods) <> colon
          <+> lbrace
            $+$ nest 2 (vcat (
              map
                (\p ->
        ...
        childPath node =
          N.mapPreOrder2
            (\pos _ -> [t "." <> t (childCallLab pos) <> parens (empty)])
            (\n' -> n')
            node
                  ...
                  (\(call, n) ->
                    ...
                    <> parens(
                      case (getNodeReturnType (Ir.definitions ir) n) of
                        Just term  -> t (termToEnumLab $ nonTerminal term)
        ...
        iff p =
          foldr1
            ...
            (map
              (\(call, n) ->
                if (isTerminal n)
                  then t (I.nId ids) <> hcat call <> t ".kind() ==" <+> t (termToEnumLab n)
                  else
