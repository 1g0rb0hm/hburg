...
data Tiling
	= Tiling
		...
		OperatorsPerArity	-- holds all operators keyed by the
							-- arity of the production (used to calc. EnumSets)
		LinkSet				-- the link set
		ProductionsPerArity	-- holds all productions with the label they produced,
							-- and are keyed by the arity of the production
...
-- | new. Constructs a Tiling from Operators and Definitions.
--		This function does quite a few things as can easily be told by
--		its lenght. Basically it unwraps definitions and extracts
				...
				mapProds (p:ps) (Tiling cl opmap lset funmap)
					...
					if (isTerm tTy)
						...
						then
							...
							let nlset = if hasLink (getNode p)
...
-- | computeLinkSet. Calculates link set for productions like:
...
-- | computeClosure. Calculate necessary values for target code closure function.
