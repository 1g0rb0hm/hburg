...
module Ast.Node (
		...
		equalIdents,hasLink,
		showAsFunction,
		-- *  Node mapping functions for easy traversal
        -- $mapping operations
...
-- | every Node should implement this class
...
data Node 
	...
	| N {
		...
		code1 	:: C.Code,
		code2	:: C.Code,
		child	:: Node,	-- link to the first child node
		code3	:: C.Code,
		sibling	:: Node,	-- points to to a sibling
		code4	:: C.Code,
		code5	:: C.Code,	-- code before link but within link block
		link	:: Node,	-- a node can have a link
...
--
-- Higher order tree traversal functions
--
...
-- | mapPreOrder. Note: The root is node is NOT processed! Processing
